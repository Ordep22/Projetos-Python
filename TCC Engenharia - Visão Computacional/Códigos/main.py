...
def set_info(detec):
    ...
    for (x, y) in detec:
        if (pos_linha + offset) > y > (pos_linha - offset):
            carros += 1
            cv2.line(frame1, (550,0), (550,1200), (0, 127, 255), 3)
            detec.remove((x, y))
            print("objetos detectados até o momento: " + str(carros))


def show_info(frame1, dilatada):
    text = f'Objetos: {carros}'
...
carros = caminhoes = 0
cap = cv2.VideoCapture('/Users/PedroVitorPereira/PycharmProjects/CursodePython/TCC-1/Vídeos Resende - Bobinas CG/c-01-20200722-095244.ts')
